	...
	// Update position coefficients //
	  ...
	  for (int dof = 0; dof < ref_elem.num_basis(); dof++){
	    for (int dim = 0; dim < mesh.num_dim(); dim++){
	      elem_state.pos_coeffs(1, elem_gid, dof, dim) = elem_state.pos_coeffs(0, elem_gid,dof, dim)
		             + 0.5*dt*(elem_state.vel_coeffs(1, elem_gid, dof, dim) + elem_state.vel_coeffs(0,elem_gid, dof, dim) );
	    }
	  }
	}
