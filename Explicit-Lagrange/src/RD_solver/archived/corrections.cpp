...
void correction(int correction_step){
  ...
  for (int elem_gid = 0; elem_gid < mesh.num_elems(); elem_gid++){
    ...
    for (int k_dof = 0; k_dof < ref_elem.num_basis(); k_dof++){
      ...
      for (int dim = 0; dim < mesh.num_dim(); dim++){
        if (correction_step%2==0){
  
          elem_state.vel_coeffs(1, elem_gid, k_dof, dim) = elem_state.vel_coeffs(0, elem_gid, k_dof, dim)
	                                                         + (dt/lumped_mass)*(0.20833*elem_state.K_res_sum(0,node_gid,dim) + 0.33333*elem_state.K_res_sum(1,node_gid,dim) - 0.04167*elem_state.K_res_sum(2,node_gid,dim));
  
          elem_state.vel_coeffs(2, elem_gid, k_dof, dim) = elem_state.vel_coeffs(0, elem_gid, k_dof, dim)
	                                                         + (dt/lumped_mass)*(0.16667*elem_state.K_res_sum(0,node_gid,dim) + 0.66667*elem_state.K_res_sum(1,node_gid,dim) + 0.16667*elem_state.K_res_sum(2,node_gid,dim));
    
          std::cout << "vel_coeffs at corr step " << elem_state.vel_coeffs(2, elem_gid, k_dof, dim) << std::endl;	
        }// end if
        else if (correction_step%2==1){
  ...
  for (int elem_gid = 0; elem_gid < mesh.num_elems(); elem_gid++){
    ...
    for (int t_dof = 0; t_dof < ref_elem.num_dual_basis(); t_dof++){
      ...
      if (correction_step%2==0){
        ...
        elem_state.sie_coeffs(1, elem_gid, t_dof) = elem_state.sie_coeffs(0, elem_gid, t_dof)
	                                                         + (dt/dual_lumped_mass)*(0.20833*elem_state.T_res_sum(0,dual_node_gid) + 0.33333*elem_state.T_res_sum(1,dual_node_gid) - 0.04167*elem_state.T_res_sum(2,dual_node_gid) );
  
        elem_state.sie_coeffs(2, elem_gid, t_dof) = elem_state.sie_coeffs(0, elem_gid, t_dof)
	                                                         + (dt/dual_lumped_mass)*(0.16667*elem_state.T_res_sum(0,dual_node_gid) + 0.66667*elem_state.T_res_sum(1,dual_node_gid) + 0.16667*elem_state.T_res_sum(2,dual_node_gid));
      }// end if
      else if (correction_step%2==1){
