...
void get_energy_rdh(int correction_step, real_t sub_dt){
    ...
    for (int elem_gid = 0; elem_gid < mesh.num_elems(); elem_gid++){
        for(int cell_lid = 0; cell_lid < mesh.num_cells_in_elem(); cell_lid++){
            ...
            cell_state.ie(correction_step, cell_gid) = 
                cell_state.ie(0, cell_gid) +  sub_dt/cell_state.mass(cell_gid) * cell_work + sub_dt/cell_state.mass(cell_gid)*source;
            ...
            cell_state.ke(correction_step,cell_gid) = ke;


            cell_state.total_energy(correction_step, cell_gid) = ke + cell_state.ie(correction_step, cell_gid);

            cell_state.total_energy(correction_step, cell_gid) = source;
