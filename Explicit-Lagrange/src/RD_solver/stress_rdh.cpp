...
void get_stress_tensor(int step){
   ...
   for (int gauss_gid = 0; gauss_gid < mesh.num_gauss_pts(); gauss_gid++){
     for (int dim_i = 0; dim_i < mesh.num_dim(); dim_i++){
       for (int dim_j = 0; dim_j < mesh.num_dim(); dim_j++){
        	elem_state.stress_tensor(step, gauss_gid, dim_i, dim_j) =  0.0;
       }
     }
   }
   ...
   for (int gauss_gid = 0; gauss_gid < mesh.num_gauss_pts(); gauss_gid++){
     for (int dim_i = 0; dim_i < mesh.num_dim(); dim_i++){
       for (int dim_j = 0; dim_j < mesh.num_dim(); dim_j++){
 	 if (dim_i == dim_j){
        	elem_state.stress_tensor(step, gauss_gid, dim_i, dim_j) =  -1.0*mat_pt.pressure(gauss_gid);
	 } 	
	 else if (dim_i != dim_j){
		elem_state.stress_tensor(step, gauss_gid, dim_i, dim_j) = 0.0;
	 } 
