  ...
  for(int elem_gid = 0; elem_gid < mesh.num_elems(); elem_gid++){
   ...
   // --- Strong Mass ---//
    ...
    for (int vertex = 0; vertex < num_basis; vertex++){
      ...
      int const force_size = num_dim*(correction_storage-1);
      real_t force_a[force_size];
      for (int i = 0; i < force_size; i++) force_a[i] = 0.0;
      auto force = ViewCArray <real_t> (&force_a[0], num_dim, correction_storage-1);
	...
	//std::cout << force(dim) << std::endl;
      ...
      int vel_bar_size = num_dim*(correction_storage-1);
      real_t vel_bar_a[vel_bar_size];
      for (int i = 0; i < vel_bar_size; i++) vel_bar_a[i] = 0.0;
      auto vel_bar = ViewCArray <real_t> ( &vel_bar_a[0], num_dim, correction_storage-1 );

      int Q_size = num_dim*(correction_storage-1);
      real_t Q_a[Q_size];
      for (int i = 0; i < Q_size; i++) Q_a[i] = 0.0;
      auto Q = ViewCArray <real_t> ( &Q_a[0], num_dim, correction_storage-1 );
