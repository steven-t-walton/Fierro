    ...
    for (int vertex = 0; vertex < ref_elem.num_basis(); vertex++){
      ...
      for (int dim = 0; dim < num_dim; dim++){
        for (int elems_in_vert = 0; elems_in_vert < num_elems_in_vert; elems_in_vert++){
          ...
          //elem_state.K_res_sum(t_step, node_gid, dim) += nodal_res(node_gid, elems_in_vert_gid, dim);
          elem_state.K_res_sum(t_step, node_gid, dim) += limited_res(elems_in_vert_gid, vertex, dim);       
